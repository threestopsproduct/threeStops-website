...
export const validateNewUser = (name, phone, email, password) => {
  if (!name) {
...
export const ValidateLoginFields = (phone, password) => {
  if (!password) {
    return "password is required";
  }
  return ValidateSingleField(phone);
}
...
export const ValidateResetPassword = (password1, password2) => {
  if (!password1) {
...
export var Creditly = (function () {
  ...
  var CardTypeListener = (function () {
    ...
    var determineCardTypeImage = function (value) {
      if (/^(34|37)/.test(value)) {
    ...
    var changeCardType = function (numberSelector, cardTypeSelector, cardAvatarSelector) {
      $(numberSelector).on("changed_input keypress keydown keyup", function (e) {
        ...
        var cardImg = determineCardTypeImage(getNumber(data));
        $(cardTypeSelector).text(cardType);
        $(cardAvatarSelector).attr('src', cardImg);
      });
    };
  ...
  var initialize = function (expirationSelector, creditCardNumberSelector, cvvSelector, cardTypeSelector, cardAvatarSelector, options) {
    ...
    CardTypeListener.changeCardType(creditCardNumberSelector, cardTypeSelector, cardAvatarSelector);
...
// mobile card validation

export var CreditlyMobi = (function () {
  var getInputValue = function (e, selector) {
    var inputValue = $.trim($(selector).val());
    inputValue = inputValue + String.fromCharCode(e.which);
    return getNumber(inputValue);
